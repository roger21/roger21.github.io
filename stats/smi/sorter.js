...
http://www.robertobizzarri.net/blog/tablesort_files/
  ...
  fdTableSort = {
    ...
    removeTableCache: function(tableId) {
      ...
      for(var i = 0, th; th = ths[i]; i++) {
        ...
        if(span.length) {
          span[0].onkeydown = span[0].onclick = null
        }
        th.onclick = th.onselectstart = th = span = null
      }
    },
    ...
    init: function(tableId) {
      ...
      var c, ii, len, colMatch, showOnly, match, showArrow, columnNumSortObj, obj, workArr, headers, thtext, span1clone, multi, colCnt, cel, allRowArr, rowArr, sortableTable, celCount, colspan, rowspan, rowLength;
      var span1 = document.createElement("span");
      span1.className = "fdTableSortTrigger";
        ...
        for(c = 0; c < workArr.length; c++) {
          for(i = 0; i < workArr[c].length; i++) {
            ...
            if(workArr[c][i].className.match("sortable")) {
              ...
              thtext = fdTableSort.getInnerText(workArr[c][i], true, false);
              thtitle = fdTableSort.getInnerText(workArr[c][i], true, true);
              ...
              if(showArrow) {
                workArr[c][i].appendChild(span2.cloneNode(false))
              }
        ...
        if("active" in columnNumSortObj) {
          ...
          for(col in columnNumSortObj) {
            ...
            if(showOnly) {
              ...
              if(showArrow) {
                span2 = fdTableSort.thNode.getElementsByTagName("span")[1];
                if(span2.firstChild) {
      ...
      fdTableSort.thNode = span1clone = span1 = span2 = columnNumSortObj = thNode = tbl = allRowArr = rowArr = null
    },
    ...
    prepareTableData: function(table) {
      ...
      for(row = 0; row < numberOfRows; row++) {
        ...
        for(var tmp = 0, th; th = sortableColumnNumbers[tmp]; tmp++) {
          ...
          td = tds[col];
          txt = fdTableSort.getInnerText(td, false, false) + " ";
          txt = txt.replace(/^\s+/, "").replace(/\s+$/, "");
          if(th.className.search(/sortable-date/) != -1) {
            txt = fdTableSort.dateFormat(txt, th.className.search(/sortable-date-dmy/) != -1)
          } else {
            if(th.className.search(/sortable-numeric|sortable-currency/) != -1) {
                  ...
                  if(th.className.search(/sortable-([a-zA-Z\_]+)/) != -1) {
                    ...
                    if(txt != "") {
                      ...
                      if(fdTableSort.dateFormat(txt) != 0) {
                        ...
                        if(txt.search(fdTableSort.regExp_Number) != -1 || txt.search(fdTableSort.regExp_Currency) != -1) {
    ...
    initSort: function(noCallback, ident) {
        ...
        for(var i = 0; i < l; i++) {
          ...
          if(th.className.match(/sortable-(numeric|currency|date|keep)/)) {
            ...
            if(th.className.match("sortable-text")) {
              ...
              if(th.className.search(/sortable-([a-zA-Z\_]+)/) != -1 && th.className.match(/sortable-([a-zA-Z\_]+)/)[1] in window) {
        ...
        if((lastPos == fdTableSort.pos && !identical) || (thNode.className.search(/sortable-keep/) != -1 && lastPos == -1)) {
          ...
          if(!identical) {
            if(thNode.className.match(/sortable-(numeric|currency|date|keep)/)) {
              ...
              if(thNode.className.match("sortable-text")) {
                dataObj.data.sort(fdTableSort.sortText)
              } else {
                if(thNode.className.search(/sortable-([a-zA-Z\_]+)/) != -1 && thNode.className.match(/sortable-([a-zA-Z\_]+)/)[1] in window) {
      ...
      if(!dataObj.noArrow) {
        var span = fdTableSort.thNode.getElementsByTagName("span")[1];
        if(span.firstChild) {
          span.removeChild(span.firstChild)
        }
    ...
    getInnerText: function(el, allowBrTags, alt) {
      ...
      for(i = el.firstChild; i; i = i.nextSibling) {
        if(allowBrTags && i.nodeName && i.nodeName == "BR") {
          txt += "<br>"
        } else {
          if(i.nodeType == 3) {
            txt += i.nodeValue
            ...
            if(i.nodeType == 1) {
              if(alt && /img/i.test(i.nodeName)) {
                txt += i.alt
              } else {
                txt += fdTableSort.getInnerText(i, allowBrTags, alt)
              }
            }
          }
        }
