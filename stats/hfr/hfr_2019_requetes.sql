...
(select rank() over (order by count(*) desc, trim(lower(replace(town, '-', ' '))) asc) rang, trim(lower(replace(town, '-', ' '))) ville, rank() over (order by trim(lower(replace(town, '-', ' '))) asc) ville_order, count(*) "nombre de comptes hfr" from p where town is not null and trim(town) != '' group by trim(lower(replace(town, '-', ' '))) order by count(*) desc, trim(lower(replace(town, '-', ' '))) asc limit 500) a left outer join
(select rank() over (order by count(*) desc, trim(lower(replace(town, '-', ' '))) asc) rang, trim(lower(replace(town, '-', ' '))) ville, count(*) "nombre de comptes hfr actifs en 2019" from p where lastpostdate >= '2019-01-01' and town is not null and trim(town) != '' group by trim(lower(replace(town, '-', ' '))) order by count(*) desc, trim(lower(replace(town, '-', ' '))) asc) b using (ville) order by a.rang
